...
/**
 * This class implements a black top-hat operation by rotational morphological processing
 *
...
public class Bth {
	...
	String msg = "Black Top-hat... " + param.CurImg() + "/" + param.TotalImg();
	IJ.showStatus(msg);
	...
	for (int y=0; y<param.ImgSizeY; y++) {
	    for (int x=0; x<param.ImgSizeX; x++) {
		DiffImg[y][x] = ClosnImg[y][x] - param.InImg[y][x];
		if(DiffImg[y][x] < 0)
		    DiffImg[y][x] = 0;
	    }
	}
